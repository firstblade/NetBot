...
CNetBotDlg::CNetBotDlg(CWnd* pParent) : CDialog(CNetBotDlg::IDD, pParent)
{
...
BOOL CNetBotDlg::DestroyWindow()
{
	...
	{
		SOCKET ClientSocket = (SOCKET)m_OnLineList.GetItemData(i);
...
DWORD CNetBotDlg::ThreadAccept()
{
	...
	{
		SOCKET ClientSocket = accept(m_ListenSock, (LPSOCKADDR)&ClientAddr, &addr_size); // accept a socket
...
int CNetBotDlg::checkfun()
{
	...
	{
		DWORD socket = m_OnLineList.GetItemData(i);
...
void CNetBotDlg::OnRclickListOnline(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (iCurrSel >= 0)
	{
		...
		CMenu *pSubMenu = OnLineMenu.GetSubMenu(0);
		CPoint oPoint;
		GetCursorPos(&oPoint);
