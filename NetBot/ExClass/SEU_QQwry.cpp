...
int SEU_QQwry::GetRecordCount()//得到总的IP记录数
{
	if (!m_bOpen) return 0;
...
void SEU_QQwry::SaveToFile(CString Name)
{
	...
	{
		int ioff = GetStartIPInfo(m_i);//得到开始的IP信息
...
{
	int index = -1;
	DWORD dwInputIP = IPtoDWORD(szIP);
	int iB = 0;
	int iE = GetRecordCount() - 1;
	int iT = iE / 2;

	while (iB < iE)
	{
		DWORD dwStartIP = GetSIP(iT);
...
DWORD SEU_QQwry::IPtoDWORD(CString szIP)
{
	...
	szIP += ".";
	memset(b, 0, 4);
	int iLen = szIP.GetLength();
	int iXB = 0;

	for (int i = 0; i < iLen; i++)
	{
		char ch = szIP.GetAt(i);
		szTemp += ch;
		if (ch == '.')
		{
			b[iXB] = _ttoi(szTemp);
	...
	DWORD iIP = b[0] * 256 * 256 * 256 + b[1] * 256 * 256 + b[2] * 256 + b[3];
	return iIP;
}
